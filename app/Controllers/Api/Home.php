...
class Home extends BaseController
{
	...
	public function otpVerify()
	{
		...
		if ($form_code == $user_info['otp']) {
			log_message('debug' , "mobile view testing=> ".json_encode(session_id())) ;
			$alphabet = 'abcdefghijklmnopqrstuvwy1234567890?/(){}[]#*ABCDEFGHIJKLNM';
	...
	public function logout()
	{
		if(isset($_GET['id'])){
			...
			$token = strrev($user['bearer_token']) ;
			$users->update($_GET['id'], [
				'bearer_token' => $token,
			]);
			return json_encode('Successfully Logout');
		}else{
	...
	public function sendOtp_for_api($user_id)
	{
		...
		$users = new Users();
		$user_data = $users->getrow($user_id);
	...
	public function verify()
	{
		...
		// log_message('debug' , 'POST Data =>'.$_POST['test']);
		// session_start();
		$wrong_data = array(
			'status' => 'false',
		...
		$deleteUser = array(
			'status' => 'false',
			'message' => '* Your account has been deleted. For more information, please contact us at support@nvestclients.com'
		);
		...
		if ($user_data != NULL) {
			if ($user_data['isDeleted'] == 'Y' || $user_data['userTypeId'] == 3) {
				...
				if (password_verify($form_password, $user_data['password'])) {
					...
					$this->sendOtp_for_api($user_data['id']);
				   	return json_encode($user_data['id']);
					// if(isset($_POST['call']) && $_POST['call'] == 'MA'){
					// 	$this->sendOtp_for_api($user_data['id']);
				   	// 	return json_encode($user_data['id']);
					// }else{
	...
	public function saveSignupData()
	{
		...
		if ($response_email == NULL) {
			...
			if (isset($_POST)) {
                ...
                $curl = curl_init();                
                curl_setopt($curl, CURLOPT_URL, "https://nvestportal.com/user_create");
				...
				return json_encode('Record Successfully Save');
			}
		} else {
	...
	{
		echo $uniqueCode = 'a74ec6fcae24ee06051f78b5c29ce71e';
		exit;
